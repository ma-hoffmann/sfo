...
public class FieldsView extends VerticalLayout {
    ...
    private void createMenuBar() {
        ...
        this.menuBar.addItem("Löschen", e -> {
            new DeleteFieldConfirmationDialog(this.grid.getSelectionModel().getFirstSelectedItem().get(),
                    this::deleteField).open();
        });
        add(this.menuBar);
    }
    ...
    private void saveField(final FieldSaveEvent event) {
        try {
            ...
            Notification
                    .show(String.format("Sportplatz %s wurde geändert! Bitte neu laden.", event.getField().getName()))
